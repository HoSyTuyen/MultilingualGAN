...
from networkv2.network import GPPatchMcResDis_yaxing, GPPatchMcResDis
from FID.fid_score import calculate_fid_given_paths
from pathlib import Path

from tqdm import tqdm

from networkv2.VDCNN import GeneratorVDCNN
from networkv2.ResNet import GeneratorResNet
...
class HWGAN(object):
    ...
    def build_generator(self):
        ...
        elif self.config['generator_arch'] == 'VDCNN':
            G = GeneratorVDCNN(nf = self.config['ngf'])
        elif self.config['generator_arch'] == 'ResNet':
            G = GeneratorResNet(nf = self.config['ngf'])
        else:
    ...
    def per_stage(self):
        ...
        FID_log = open(self.config['project_name'] +'_results' + '/FID_log.txt', 'w')
        maxFID = 0
        start_time = time.time()
